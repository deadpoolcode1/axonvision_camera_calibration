...
class CameraPreviewScreen(QWidget):
    ...
    camera_removed = Signal(int)  # Emitted when a camera is removed
    ...
    def _setup_ui(self):
        ...
        screen_label.setToolTip("Current screen in the calibration workflow")
        header_layout.addWidget(screen_label)

        title = QLabel("Intrinsic Calibration")
        title.setObjectName("title")
        title.setToolTip("Intrinsic calibration determines each camera's internal parameters (focal length, distortion)")
        header_layout.addWidget(title)

        step_label = QLabel("Step 2 of 6")
        step_label.setObjectName("step_indicator")
        step_label.setToolTip("Progress through the calibration workflow")
        ...
        """)
        instructions_frame.setToolTip("Instructions for completing this step")
        ...
        instructions_text = QLabel(
            ...
            "You can also remove cameras from the configuration if needed."
        )
        ...
        table_header.setToolTip("List of all configured cameras with their calibration status")
        ...
        self.cancel_btn.setToolTip("Return to the platform configuration screen to modify camera settings")
        ...
        self.next_btn.setToolTip("Proceed to extrinsic calibration. All cameras must have intrinsic calibration completed first.")
    ...
    def _setup_camera_table(self):
        ...
        columns = ['#', 'Camera ID', 'Type', 'Model', 'Position', 'IP Address', 'Intrinsic Status', 'Actions']
        self.camera_table.setColumnCount(len(columns))
        self.camera_table.setHorizontalHeaderLabels(columns)
        self.camera_table.setToolTip("Camera configuration and calibration status")
        ...
        header.setSectionResizeMode(4, QHeaderView.Stretch)           # Position
        ...
        self.camera_table.setColumnWidth(7, 160)
    ...
    def _populate_camera_table(self):
        ...
        for camera in self.config.cameras:
            ...
            num_item.setToolTip(f"Camera number {camera.camera_number} in the configuration")
            ...
            id_item.setToolTip(f"Unique identifier for this camera: {camera.camera_id}")
            ...
            type_item.setToolTip(f"Camera type: {camera.camera_type}")
            ...
            model_item.setToolTip(f"Hardware model: {camera.camera_model}")
            ...
            pos_item.setToolTip(f"Mounting position on vehicle: {camera.mounting_position}")
            ...
            ip_item.setToolTip(f"Network address: {camera.ip_address}")
            ...
            if has_intrinsic:
                ...
                status_item.setToolTip("Intrinsic calibration complete. Camera is ready for extrinsic calibration.")
            else:
                status_item = QTableWidgetItem("✗ Not Calibrated")
                status_item.setForeground(Qt.red)
                status_item.setToolTip("Intrinsic calibration required. Click 'Calibrate' to start.")
            ...
            calibrate_btn.setToolTip(f"Run intrinsic calibration for {camera.camera_id}. Uses ChArUco board to determine camera parameters.")
            ...
            remove_btn.clicked.connect(
                lambda checked, cam_num=camera.camera_number: self._on_remove_clicked(cam_num)
            )
    ...
    def _on_remove_clicked(self, camera_number: int):
        ...
        if len(self.config.cameras) <= 1:
            QMessageBox.warning(
                self,
                "Cannot Remove Camera",
                "At least one camera must be configured.\n\n"
                "Possible solutions:\n"
                "  1. Add a new camera before removing this one\n"
                "  2. Go back to configuration to manage cameras"
            )
            return
        ...
        reply = QMessageBox.question(
            ...
            f"Position: {camera.mounting_position}\n"
            f"IP Address: {camera.ip_address}\n\n"
        ...
        if reply == QMessageBox.Yes:
            logger.info(f"Removing camera: {camera.camera_id}")
            # Find index and remove
            index = next((i for i, c in enumerate(self.config.cameras)
                         if c.camera_number == camera_number), None)
            if index is not None:
                self.config.remove_camera(index)
    ...
    def _on_calibrate_clicked(self, camera_number: int):
        ...
        logger.info(f"Starting intrinsic calibration for camera: {camera.camera_id}")
    ...
    def _on_calibration_completed(self, camera_number: int, success: bool, message: str):
        ...
        if success:
            logger.info(f"Calibration completed successfully for camera number {camera_number}")
            # Refresh the table to update intrinsic status
            self._populate_camera_table()
        else:
            logger.warning(f"Calibration failed for camera number {camera_number}: {message}")
    ...
    def _on_next_clicked(self):
        ...
        if missing_intrinsics:
            logger.warning(f"Missing intrinsic calibration for cameras: {missing_intrinsics}")
            QMessageBox.warning(
                self,
                ...
                "\n\nPossible solutions:\n"
                "  1. Click 'Calibrate' for each camera listed above\n"
                "  2. Ensure the ChArUco board is visible and well-lit\n"
                "  3. Remove uncalibrated cameras if they are not needed\n"
                "  4. Go back to configuration to verify camera connections"
            )
            return

        # All cameras calibrated, proceed
        logger.info("All cameras calibrated, proceeding to extrinsic calibration")
    ...
    def verify_cameras(self):
        ...
        """
        logger.debug("Verifying cameras and refreshing table")
        self._populate_camera_table()
